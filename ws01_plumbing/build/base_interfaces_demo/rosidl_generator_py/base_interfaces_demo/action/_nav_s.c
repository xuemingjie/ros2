...
bool base_interfaces_demo__action__nav__goal__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // goal_thrta
    PyObject * field = PyObject_GetAttrString(_pymsg, "goal_thrta");
    if (!field) {
      return false;
    }
    assert(PyFloat_Check(field));
    ros_message->goal_thrta = (float)PyFloat_AS_DOUBLE(field);
    Py_DECREF(field);
  }
...
PyObject * base_interfaces_demo__action__nav__goal__convert_to_py(void * raw_ros_message)
{
  ...
  {  // goal_thrta
    PyObject * field = NULL;
    field = PyFloat_FromDouble(ros_message->goal_thrta);
    {
      int rc = PyObject_SetAttrString(_pymessage, "goal_thrta", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
